...
class MiniDroneBtAdapter extends EventEmitter {
    ...
    cleanPeripheral() {
        ...
        // Remove listener for battery notifications.
        this.getCharacteristic(BATTERY_KEY).removeListener('data', (data, isNotification) => {
            this.onBatteryStatusChange(data, isNotification);
        });

        // Remove a listener for flight status changes
        this.getCharacteristic(FLIGHT_STATUS_KEY).removeListener('data', (data, isNotification) => {                     
            this.onFlightStatusChange(data, isNotification);
        });
